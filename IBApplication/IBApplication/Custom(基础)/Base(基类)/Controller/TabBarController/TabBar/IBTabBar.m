...
@property (nonatomic, strong) NSMutableArray <IBTabBarItem *> *items;
// 上次选择的item
@property (nonatomic, strong) IBTabBarItem *preSelectItem;
// TabBar背景图
@property (nonatomic, strong) UIImageView *backgroundImageView;

@end

@implementation IBTabBar
...
// 点击的tabbarItem
- (void)click_tabBarItem:(IBTabBarItem *)sender{
    NSInteger clickIndex = sender.tag - IBTabBarTag;
...
- (void)itemDidLayoutBulge{
    ...
    [self.items enumerateObjectsUsingBlock:^(IBTabBarItem * _Nonnull item, NSUInteger idx, BOOL * _Nonnull stop) {
        ...
        switch (item.itemModel.bulgeStyle) {
            case IBTabBarBulgeNormal:
                break;         // 无 默认
            case IBTabBarBulgeCircular: { // 圆形
...
// 设置items
- (void)setItemModels:(NSArray<IBTabBarItemModel *> *)itemModels {
    _itemModels = itemModels;
    [itemModels enumerateObjectsUsingBlock:^(IBTabBarItemModel * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        IBTabBarItem *item = [[IBTabBarItem alloc] initWithModel:obj]; // 模型转成实例
        item.itemIndex = idx; // 交付索引
        item.tintColor = self.tintColor;
        item.tag = IBTabBarTag + idx; // 区分Tag
...
- (NSArray<IBTabBarItem *> *)tabBarItems { // 对外只读
    return self.items;
}

- (IBTabBarItem *)currentSelectItem { // 直接用只读的属性
...
- (NSMutableArray<IBTabBarItem *> *)items {
    if (!_items) {
        _items = @[].mutableCopy;
    }
    return _items;
