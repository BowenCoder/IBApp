...
@implementation IBPopupControllerTransitioningContext

- (instancetype)initWithContainerView:(UIView *)containerView action:(IBPopupControllerTransitioningAction)action
{
    if (self = [super init]) {
...
CGFloat const IBPopupBottomSheetExtraHeight = 80;

static NSMutableSet *_retainedPopupControllers;

@protocol IBPopupNavigationTouchEventDelegate <NSObject>

- (void)popupNavigationBar:(IBPopupNavigationBar *)navigationBar touchDidMoveWithOffset:(CGFloat)offset;
- (void)popupNavigationBar:(IBPopupNavigationBar *)navigationBar touchDidEndWithOffset:(CGFloat)offset;

@end

@interface IBPopupNavigationBar (IBInternal)

@property (nonatomic, weak) id<IBPopupNavigationTouchEventDelegate> touchEventDelegate;

@end

@interface UIViewController (IBInternal)

@property (nonatomic, weak) IBPopupController *popupController;

@end

@interface IBPopupContainerViewController : UIViewController

@end

@implementation IBPopupContainerViewController
...
@interface IBPopupController () <UIViewControllerTransitioningDelegate, UIViewControllerAnimatedTransitioning, IBPopupNavigationTouchEventDelegate>

@end

@implementation IBPopupController
{
    IBPopupContainerViewController *_containerViewController;
    NSMutableArray *_viewControllers; // <UIViewController>
    UIView *_contentView;
    UILabel *_defaultTitleLabel;
    IBPopupLeftBarItem *_defaultLeftBarItem;
    NSDictionary *_keyboardInfo;
    BOOL _observing;
    
    // Built-in transitioning
    IBPopupControllerTransitioningSlideVertical *_transitioningSlideVertical;
    IBPopupControllerTransitioningFade *_transitioningFade;
}
...
- (void)setupObservers
{
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(firstResponderDidChange) name:IBPopupFirstResponderDidChangeNotification object:nil];
}
...
#pragma mark - IBPopupController present & dismiss & push & pop
...
- (void)updateNavigationBarAnimated:(BOOL)animated
{
    ...
    [_defaultLeftBarItem setType:_viewControllers.count > 1 ? IBPopupLeftBarItemArrow : IBPopupLeftBarItemCross
...
- (void)layoutContainerView
{
    ...
    if (self.style == IBPopupStyleBottomSheet) {
        containerViewY = _containerViewController.view.bounds.size.height - containerViewHeight;
        containerViewHeight += IBPopupBottomSheetExtraHeight;
    }
...
- (void)setup
{
    _containerViewController = [IBPopupContainerViewController new];
    ...
    _transitioningSlideVertical = [IBPopupControllerTransitioningSlideVertical new];
    _transitioningFade = [IBPopupControllerTransitioningFade new];
}
...
{
    IBPopupNavigationBar *navigationBar = [IBPopupNavigationBar new];
    ...
    _defaultTitleLabel = [UILabel new];
    _defaultLeftBarItem = [[IBPopupLeftBarItem alloc] initWithTarget:self action:@selector(leftBarItemDidTap)];
}

- (void)leftBarItemDidTap
{
    switch (_defaultLeftBarItem.type) {
        case IBPopupLeftBarItemCross:
            [self dismiss];
            break;
        case IBPopupLeftBarItemArrow:
...
#pragma mark - IBPopupFirstResponderDidChangeNotification
...
- (IBPopupControllerTransitioningContext *)convertTransitioningContext:(id <UIViewControllerContextTransitioning>)transitionContext
{
    IBPopupControllerTransitioningAction action = IBPopupControllerTransitioningActionPresent;
    if ([transitionContext viewControllerForKey:UITransitionContextToViewControllerKey] != _containerViewController) {
        action = IBPopupControllerTransitioningActionDismiss;
    }
    return [[IBPopupControllerTransitioningContext alloc] initWithContainerView:_containerView action:action];
}

- (NSTimeInterval)transitionDuration:(id <UIViewControllerContextTransitioning>)transitionContext
{
    IBPopupControllerTransitioningContext *context = [self convertTransitioningContext:transitionContext];
    switch (self.transitionStyle) {
        case IBPopupTransitionStyleSlideVertical:
            return [_transitioningSlideVertical popupControllerTransitionDuration:context];
        case IBPopupTransitionStyleFade:
            return [_transitioningFade popupControllerTransitionDuration:context];
        case IBPopupTransitionStyleCustom:
            NSAssert(self.transitioning, @"transitioning should be provided if it's using IBPopupTransitionStyleCustom");
...
- (void)animateTransition:(id <UIViewControllerContextTransitioning>)transitionContext
{
    ...
    IBPopupControllerTransitioningContext *context = [self convertTransitioningContext:transitionContext];
    id<IBPopupControllerTransitioning> transitioning = nil;
    switch (self.transitionStyle) {
        case IBPopupTransitionStyleSlideVertical:
    ...
    NSAssert(transitioning, @"transitioning should be provided if it's using IBPopupTransitionStyleCustom");
...
#pragma mark - IBPopupNavigationTouchEventDelegate

- (void)popupNavigationBar:(IBPopupNavigationBar *)navigationBar touchDidMoveWithOffset:(CGFloat)offset
{
    [_containerView endEditing:YES];
    
    if (self.style == IBPopupStyleBottomSheet && offset < -IBPopupBottomSheetExtraHeight) {
        return;
    }
...
{
    if (offset > 150) {
        IBPopupTransitionStyle transitionStyle = self.transitionStyle;
        self.transitionStyle = IBPopupTransitionStyleSlideVertical;
