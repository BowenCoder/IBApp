...
- (void)commonInit {
    ...
    // default values
    _scrollDirection = MBAutoScrollDirectionLeft;
...
- (void)setScrollDirection:(MBAutoScrollDirection)direction {
...
- (void)scrollLabelIfNeeded {
    ...
    BOOL doScrollLeft = (self.scrollDirection == MBAutoScrollDirectionLeft);
...
- (void)applyGradientMaskForFadeLength:(CGFloat)fadeLength enableFade:(BOOL)fade {
    ...
    if (fadeLength) {
        ...
        if (!fade) switch (self.scrollDirection) {
                case MBAutoScrollDirectionLeft:
                    leftFadePoint = @0;
                    break;

                case MBAutoScrollDirectionRight:
                    leftFadePoint = @0;
                    rightFadePoint = @1;
                    break;
            }
