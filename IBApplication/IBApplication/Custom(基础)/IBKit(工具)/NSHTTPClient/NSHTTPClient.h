...
typedef void(^NSHTTPClientSuccess)(id JSON);
typedef void(^NSHTTPClientError)(NSError *error);

typedef NS_ENUM(NSInteger, NSHTTPClientNetworkStatus){
    NSHTTPClientNetworkUnknown       = -1,
    NSHTTPClientNetworkNotReachable  = 0,
    NSHTTPClientNetworkViaWWAN       = 1,
    NSHTTPClientNetworkViaWiFi       = 2
};

@interface NSHTTPClient : AFHTTPSessionManager

@property (nonatomic, assign) NSHTTPClientNetworkStatus networkStatus;
