...
NSString* const RMSKRestoreShouldAddStorePayment = @"RMSKRestoreShouldAddStorePayment";
...
{
    [self addPayment:productIdentifier user:nil requestData:nil success:successBlock failure:failureBlock];
}
...
- (void)addStoreObserver:(id<RMStoreObserver>)observer
{
    ...
    [self addStoreObserver:observer selector:@selector(storePaymentShouldAddStorePayment:) notificationName:RMSKRestoreShouldAddStorePayment];
}
...
- (void)removeStoreObserver:(id<RMStoreObserver>)observer
{
    ...
    [[NSNotificationCenter defaultCenter] removeObserver:observer name:RMSKRestoreShouldAddStorePayment object:self];
}
...
{
    if (product) {
        NSDictionary *userInfo = @{ RMStoreNotificationProducts : @[product] };
        [[NSNotificationCenter defaultCenter] postNotificationName:RMSKRestoreShouldAddStorePayment object:self userInfo:userInfo];
    }
...
- (void)finishTransaction:(SKPaymentTransaction *)transaction queue:(SKPaymentQueue*)queue
{
    ...
    // https://stackoverflow.com/questions/3139280/transaction-comes-back-after-finishtransaction-has-been-called-on-it#answer-8250274
