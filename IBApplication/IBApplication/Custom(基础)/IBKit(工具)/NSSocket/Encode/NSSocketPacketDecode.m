...
@interface NSSocketPacketDecode ()
...
- (NSInteger)decodeDownPacket:(NSData *)downPacket output:(id<NSSocketDecoderOutputProtocol>)output {
    ...
    while (![NSHelper isEmptyData:downPacket] && downPacket.length > _countOMMengthByte + headIndex) {
        // 包长度
        ...
        NSDownloadDataPacket *downPacket = [[NSDownloadDataPacket alloc] initWithData:packetData];
