...
#import "IBSocketService.h"
#import "IBSocketClient.h"
#import "IBNetworkStatus.h"

NSString * const kIBSocketAuthSuccessNotification = @"kIBSocketAuthSuccessNotification";
NSString * const kIBSocketErrorConnectedNotification = @"kIBSocketErrorConnectedNotification";

static IBSocketService *service = nil;

@interface IBSocketService () <IBSocketClientDelegate>

@property (nonatomic, assign) BOOL hostRequesting;
@property (nonatomic, strong) IBSocketClient *socketClient;
@property (nonatomic, strong) IBSocketConfig *connectConfig;

@end

@implementation IBSocketService

+ (instancetype)sharedService {
    
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        service = [[IBSocketService alloc] init];
    });
    return service;
}
...
- (void)startConnectSocket {
    ...
    if (self.connectConfig) {
        ...
        NSLog(@"开始连接socket...");
        if (!self.socketClient) {
            self.socketClient = [[IBSocketClient alloc] initWithConnectConfig:self.connectConfig];
            self.socketClient.heartbeatPacket = [[IBUploadDataPacket alloc] initWithPacketType:IBSocket_Msg_Heartbeat content:nil];
...
- (void)addNetworkingStatusNotification {
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(onReceiveReachabilityChangedNotification:) name:kIBReachabilityChangedNotification object:nil];
}

- (void)removeNetworkingStatusNotification {
    
    [[NSNotificationCenter defaultCenter] removeObserver:self name:kIBReachabilityChangedNotification object:nil];
}
...
- (void)sendPacketWithPacketType:(IBSocketMsgType)packetType content:(NSDictionary *)content {
    ...
    IBUploadDataPacket *authPacket = [[IBUploadDataPacket alloc] initWithPacketType:packetType content:content];
